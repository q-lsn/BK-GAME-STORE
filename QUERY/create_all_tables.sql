...
CREATE TABLE [PRODUCT](
	...
	base_game_id		varchar(6)		NOT NULL,
	base_subgame_id		varchar(6)		,
	product_price		decimal(10,2)	DEFAULT 0.00 CHECK (product_price >= 0),
	tradeable			bit				NOT NULL CHECK (tradeable IN (0, 1)),
);
...
CREATE TABLE TRADE_BOX (
	...
	trade_status varchar(7) DEFAULT 'Pending' CHECK (trade_status IN ('Pending', 'Success', 'Failed')),
...
CREATE TABLE WORKSHOP_ITEMS(
  ...
  FOREIGN KEY (create_user) REFERENCES [USER](account_id) ON DELETE SET NULL
);
...
CREATE TABLE _SUBCRIBE(
  ...
  FOREIGN KEY (user_id) REFERENCES [USER](account_id) ON DELETE CASCADE,
  FOREIGN KEY (workshop_id) REFERENCES WORKSHOP_ITEMS(workshop_id) ON DELETE CASCADE
);
...
CREATE TABLE _MODS(
  ...
  compatible BIT NOT NULL,

  PRIMARY KEY (workshop_id), 
  FOREIGN KEY (game_applied) REFERENCES [GAMES](game_id) ON DELETE CASCADE,
  FOREIGN KEY (workshop_id) REFERENCES WORKSHOP_ITEMS(workshop_id) ON DELETE CASCADE
);
...
CREATE TABLE MODS_CHANGELOG(
  ...
  FOREIGN KEY (workshop_id) REFERENCES _MODS(workshop_id) ON DELETE CASCADE
);
...
CREATE TABLE ARTWORK_STYLE(
  ...
  FOREIGN KEY (workshop_id) REFERENCES ARTWORKS(workshop_id) ON DELETE CASCADE
);